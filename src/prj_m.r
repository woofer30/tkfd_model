...
base$M_PGDP_GR= dexpd(
  base$M_PGDP_GR_T,
  as.numeric(base$M_PGDP_GR_T[index(base) == 2013])
  + base$M_CPI_GR
  - as.numeric(base$M_CPI_GR[index(base) == 2013]),
  2013
)
...
###CPI変化率で引き延ばした率
base$M_WAGE_GR= dexpd(base$M_WAGE_GR_T,
                        (base$M_TFP_GR) / (1 - M_BUNPAI) + base$M_CPI_GR,
                        2013)
...
base$M_POP_15_64_GR_T= base$M_POP_15_64 / lag(base$M_POP_15_64, k = -1) -
  1
...
## 実質GDP成長率 伸長
base$M_GDP_R_GR= dexpd(base$M_GDP_R_GR_T,
                         (base$M_TFP_GR) / (1 - M_BUNPAI) + base$M_POP_15_64_GR,
                         2013)
